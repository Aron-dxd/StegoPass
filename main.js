        ...
        for (let y = 1; y < height - 1; y++) {
            ...
            for (let dx = -1; dx <= 1; dx++) {
                for (let dy = -1; dy <= 1; dy++) {
                    ...
                    r += imageData[pixelIndex] * kernel[dy + 1][dx + 1];
                    g += imageData[pixelIndex + 1] * kernel[dy + 1][dx + 1];
                    b += imageData[pixelIndex + 2] * kernel[dy + 1][dx + 1];
                }
            }

            // Set the new blurred pixel values
            const newPixelIndex = (y * width + x) * 4;
            blurredData.data[newPixelIndex] = r;
            blurredData.data[newPixelIndex + 1] = g;
            blurredData.data[newPixelIndex + 2] = b;
            blurredData.data[newPixelIndex + 3] = 255; // Alpha
        }
    }
...
function encodeImage(image) {
    ...
    // Apply Gaussian Blur
    const blurredData = applyGaussianBlur(imageData.data, width, height);
